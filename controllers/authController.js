...
module.exports.login = async (req, res, next) => {
  ...
  try {
    ...
    if (!checkUser) {
      ...
      // });
      return next(
        new ErrorResponse("Tài khoản không tồn tại trên hệ thống", 404)
      );
    }
...
module.exports.forgotpassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      // neu khong ton tai user voi email tuong ung thi tra ve loi
...
module.exports.resetPassword = async (req, res, next) => {
  // Lấy ra resetToken truyền từ params và đưa nó về mã hex
  ...
  try {
    // tìm user theo resetToken trong DB và ngày hết hạn lớn hơn hiện tại
    const user = await User.findOne({
      resetPasswordToken: resetToken,
      resetPasswordExprise: { $gt: Date.now() },
    });
    if (!user) {
      return next(new ErrorResponse("Reset Token không có hoặc hết hạn", 400));
    }
    // gán lại password mới ( lưu ý ko cần jwt vì đã làm ở User model)
    // gán undefined cho token và hạn
